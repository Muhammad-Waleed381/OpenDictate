...
export function DockButton() {
  ...
  if (flash === "inserted") {
    content = <span className="animate-od-pop text-[7px] font-bold text-green-400">✓</span>;
  } else if (flash === "error" || error) {
    content = <span className="animate-od-pop text-[7px] font-bold text-red-400">✕</span>;
    ...
    content = (
      <span className="flex gap-px">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className="size-[2px] animate-od-bounce-y bg-white"
  ...
  return (
    <button
      type="button"
      onClick={toggle}
      className={`absolute right-0 top-0 flex size-3 cursor-pointer items-center justify-center rounded-full border transition-transform hover:scale-150 ${flash === "error" || error ? "border-red-400 bg-black" : flash === "inserted" ? "border-green-400 bg-black" : active || state === "transcribing" ? "border-black bg-black" : "border-black bg-white opacity-80"}`}
